...
grid_res = [(50,50)]
model_type = ['ATSP','PMEDIAN', 'HYBRID']
training_size = [20,50]
feat_sizes = ['small', 'large']
    ...
    for train_size in training_size:
        for model in model_type:
            for feat_size in feat_sizes:
                with open(f'predictions/new_large_data_dict_westmount_with_predictions_{model}_{feat_size}_{grid[0]}_{grid[1]}_{train_size}.pkl', 'rb') as f:
                    data_dict = pickle.load(f)
                print(f'Solving benchmark problems for {model}_{feat_size}_{grid[0]}_{grid[1]}_{train_size}')
                for size in sizes:
                    for n_center in n_centers[size]:
                        for instance in tqdm(range(100)):
                            ...
                            our_dict = {
                                'prediction_dist_mat': d_mat,
                                'predicted_adjacency': adjacency,
                            ...
                            with open(f'dataset/optimization_datasets/PMEDIAN/{size}/{model}_{feat_size}_{grid[0]}_{grid[1]}_{train_size}_{instance}_{n_center}.pkl', 'wb') as f:
