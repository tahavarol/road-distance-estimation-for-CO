...
for grid in grid_res:
    for loss in losses:
        for feat_size in feat_sizes:
            with open(f'predictions/new_large_data_dict_westmount_with_predictions_{loss}_{loss}_{feat_size}_{grid[0]}_{grid[1]}.pkl', 'rb') as f:
                data_dict = pickle.load(f)
            print(f'Solving benchmark problems for {loss}_{feat_size}_{grid[0]}_{grid[1]}')
            for size in sizes:
                for n_center in n_centers[size]:
                    for instance in tqdm(range(100)):
                        ...
                        with open(f'dataset/optimization_datasets/PMEDIAN/{size}/{loss}_{feat_size}_{grid[0]}_{grid[1]}_{instance}_{n_center}.pkl', 'wb') as f:
