...
for grid in grid_res:
    ...
    for size in sizes:
        for instance in tqdm(range(100)):
            with open(f'dataset/optimization_datasets/ATSP/{size}/LKH_instance_{instance}.pkl', 'rb') as f:
                LKH_dict = pickle.load(f)
            d_mat = np.zeros((size, size))
            for i in range(size):
                for j in range(size):
                    if i != j:
                        d_mat[i, j] = data_dict['predictions'][LKH_dict['location_idx'][i]][LKH_dict['location_idx'][j]]
            cities = elkai.DistanceMatrix(d_mat)
            t_SNAP = cities.solve_tsp()
            our_dict = {
                'predicted_tour': t_SNAP,
                'predicted_length': RouteLength(t_SNAP, LKH_dict['road_dist_mat']),
                'LKH_length': LKH_dict['tour_length']
            }
            with open(f'dataset/optimization_datasets/ATSP/{size}/SNAP_{grid[0]}_{grid[1]}_{instance}.pkl', 'wb') as f:
